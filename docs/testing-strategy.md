...
Status: active strategy. Current local evidence is 109 passing backend tests, 34 passing frontend tests, passing frontend/mobile TypeScript checks, passing frontend production build, passing mobile lint and passing mobile Jest. Normal CI must not need model credentials or make paid calls.
...
| Domain logic | Coaching write eligibility, explicit attempt workflow routes, target extraction from provider/persisted evaluations, same-context baseline/retry, target-specific outcome, audience changes, ambiguous improvement, unsupported evidence, duplicate observation, profile rebuild equivalence, correlated retries and deletion/supersession behavior |
| Database | Real isolated SQLite plus PostgreSQL integration for supported release path; constraints, foreign-key enforcement, workflow route/reason persistence, commit/rollback, unique sequence/idempotency, concurrent attempts, no lost profile update, deletion and migration/backfill behavior |
| API integration | Actual multipart requests, stable response schema/status, ownership isolation, ordered session attempt history, invalid media, provider timeout/invalid JSON, persistence failure, no premature success, request retry idempotency; reload saved measurements before comparison |
...
| Frontend | Render actual API contract fixtures; setup form payload, setup context visibility across recording/review/feedback, unavailable/abstained results, workflow route visibility, durable history panel, request error/retry, comparison visibility, task preservation; fake MediaRecorder/tracks/timers for manual and automatic stop, one completion, unmount cleanup, delayed permissions and StrictMode |
